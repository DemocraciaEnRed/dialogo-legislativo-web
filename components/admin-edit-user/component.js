...
class UserEdit extends Component {
  ...
  async fetchUser(){
    const user = await (await fetch(`${API_URL}/api/v1/users/${this.props.router.query.user}`)).json()
    this.setState({
      user:user,
  ...
  handleSubmit = async (e) => {
    ...
    const updatedUser = await (await fetch(`${API_URL}/api/v1/users/${this.state.user._id}`, {
      'method': 'PUT',
      'headers': {
