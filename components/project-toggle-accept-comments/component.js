...
class ToggleAcceptComments extends Component {
  constructor(props) {
    super(props)
    this.state = {
      showAlert: false,
    ...
    this.props.setAcceptComments(this.props.project.acceptComments)
  }
  ...
  submitToggle = async () => {
    ...
    let newState = !this.props.canAcceptComments
  ...
  render() {
    const { canAcceptComments } = this.props
    const { showAlert, alertType, alertText, isLoading } = this.state
    return (
      <Wrapper>
        <Label>Aceptar comentarios</Label>
        {
          !isLoading &&
            <ToggleClick onClick={this.submitToggle}>
              {
                canAcceptComments ? <Icon icon={toggleFilled} size={40} style={{ color: 'green' }} /> : <Icon icon={toggle} size={40} style={{ color: '#4a5d84' }} />
              } {
                canAcceptComments && !isLoading ? 'Si' : 'No'
              }
            </ToggleClick>
        }
        {
...
ToggleAcceptComments.propTypes = {
  authContext: PropTypes.object,
  toggleAcceptComments: PropTypes.func,
  canAcceptComments: PropTypes.bool,
  project: PropTypes.object
}

export default WithUserContext(ToggleAcceptComments)
